...
# --- 3. Filtering & PSI Calculation ---
# Filter first to keep the Jaccard matrix calculation lean
fds <- filterExpressionAndVariability(fds, minDeltaPsi = 0.0, filter = TRUE)
fds_filtered <- fds[mcols(fds, type = "j")[, "passed"], ]

# Calculate Jaccard metrics only on filtered junctions
fitMetrics(fds_filtered) <- "jaccard"
fds_filtered <- calculatePSIValues(fds_filtered, types = "jaccard", BPPARAM = bp)

# --- 4. QC Plots (Downsampled) ---
dir.create("qc_plots", showWarnings = FALSE)
set.seed(42)
plot_idx <- sample(nrow(fds_filtered), min(nrow(fds_filtered), 30000))
fds_plot_subset <- fds_filtered[plot_idx, ]
...
message("Optimizing Hyperparameters (q) for Jaccard...")
# This step is required before calling bestQ
fds_filtered <- optimHyperParams(fds_filtered, type = "jaccard", BPPARAM = bp)
...
# --- 6. Fitting ---
message(paste0("Fitting model with q=", opt_q))
...
res_all <- results(fds_fit, type = "jaccard", padjCutoff = 1, deltaPsiCutoff = 0, minCount = 0)
